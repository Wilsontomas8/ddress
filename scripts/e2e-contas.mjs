...
try {
  ...
  const registo = await balcao.request.post(`${BASE}/api/auth/registar`, {
    data: { nome: "Cliente Percurso", email: EMAIL, telefone: "923 000 999", password: "primeira123" },
  });
  passo("Conta criada para o percurso", registo.ok(), `estado ${registo.status()}`);
  ...
  await pagina.getByText("Se existir conta com esse e-mail").waitFor({ timeout: 45000 });
  passo("Pedido de nova palavra-passe aceite", true);

  const ligacao = await pagina.locator("a[href*='/recuperar/']").first().getAttribute("href");
  ...
  const repetida = await pagina.request.post(`${BASE}/api/auth/redefinir`, { data: { codigo: caminho.split("/").pop(), password: "terceira123" } });
  passo("A mesma ligação não serve duas vezes", repetida.status() === 400, `estado ${repetida.status()}`);
  ...
  await balcao.request.post(`${BASE}/api/auth/registar`, { data: { nome: "Travão Percurso", email: outro, telefone: "923 000 998", password: "primeira123" } });
  let ultimo = 0;
  for (let i = 0; i < 9; i++) {
    const r = await pagina.request.post(`${BASE}/api/auth/entrar`, { data: { email: outro, password: "errada" }, failOnStatusCode: false });
    ultimo = r.status();
  }
  passo("Tentativas erradas travam a entrada", ultimo === 429, `última resposta ${ultimo}`);
  await balcao.close();
  ...
  const verQuemSomos = await menu.getByRole("link", { name: "Quem somos" }).isVisible();
  passo("Menu do telemóvel ocupa o ecrã e mostra as páginas", alturaMenu > 700 && verQuemSomos, `altura ${Math.round(alturaMenu)}px`);
  ...
  const larguras = await tele.evaluate(() => {
    ...
    return { galeria: g ? Math.round(g.getBoundingClientRect().width) : 0, ecra: window.innerWidth, documento: document.documentElement.scrollWidth };
  });
  passo(
    "Galeria do Quem somos cabe no telemóvel",
    larguras.galeria > 0 && larguras.galeria <= larguras.ecra && larguras.documento <= larguras.ecra,
  ...
  passo("Morada da loja no rodapé", await tele.getByRole("contentinfo").getByText("Cassenda", { exact: false }).first().isVisible());
  await tele.screenshot({ path: `${TIROS}/contas-quem-somos-telemovel.png`, fullPage: true });
  await movel.close();
} catch (e) {
