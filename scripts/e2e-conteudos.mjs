...
try {
  ...
  passo(
    "Quem somos mostra os destaques",
    await site
      .getByText("Mulheres atendidas", { exact: false })
      .first()
      .isVisible(),
  );
  ...
  passo(
    "Página da colecção lista peças",
    pecas > 0,
    `${destino}: ${pecas} ligações`,
  );
  ...
  await formMaq
    .getByRole("button", { name: "Enviar pedido de maquilhagem" })
    .click();
  const codigo = (
    await site.locator("p.num").first().innerText({ timeout: 45000 })
  ).trim();
  passo(
    "Cliente envia pedido de maquilhagem",
    /^SOL-\d{4}-\d{4}$/.test(codigo),
    codigo,
  );
  ...
  const codigoSapatos = (
    await site.locator("#sapatos p.num").first().innerText({ timeout: 45000 })
  ).trim();
  passo(
    "Cliente pede sapatos escolhidos",
    /^SOL-/.test(codigoSapatos),
    codigoSapatos,
  );
  await publico.close();

  // ------------------------------------------------ loja (administrador)
  const { contexto: ctxAdmin, pagina: admin } = await entrar(
    navegador,
    "admin@ddress.ao",
    "admin123",
  );

  await admin.goto(`${BASE}/admin/notificacoes`, {
    waitUntil: "domcontentloaded",
  });
  await hidratado(admin, "aside, nav");
  passo(
    "Loja recebe o aviso da solicitação",
    await admin.getByText(`Nova solicitação ${codigo}`).first().isVisible(),
  );
  await admin.screenshot({ path: `${TIROS}/conteudos-notificacoes.png` });

  await admin.goto(`${BASE}/admin/solicitacoes`, {
    waitUntil: "domcontentloaded",
  });
  await hidratado(admin, "aside, nav");
  ...
  await admin
    .getByText("Em contacto", { exact: true })
    .first()
  ...
  const sugeriu = await admin
    .getByText("Sugestões guardadas.")
    .waitFor({ timeout: 45000 })
    .then(
      () => true,
      () => false,
  ...
  await admin.screenshot({
    path: `${TIROS}/conteudos-solicitacao.png`,
    fullPage: true,
  });

  // Colecção nova
  await admin.goto(`${BASE}/admin/colecoes/nova`, {
    waitUntil: "domcontentloaded",
  });
  await hidratado(admin, "aside, nav");
  ...
  passo(
    "Colecção apagada deixa de existir",
    r2.status() === 404,
    `estado ${r2.status()}`,
  );

  // Slide novo na página inicial
  await admin.goto(`${BASE}/admin/conteudos/inicio`, {
    waitUntil: "domcontentloaded",
  });
  await hidratado(admin, "aside, nav");
  ...
  passo(
    "Página inicial mostra o slide novo",
    html.includes("Percurso de teste"),
  );
  ...
  passo(
    "Slide apagado sai da página inicial",
    !depois.includes("Percurso de teste"),
  );

  // Permissões: funcionário passa a só ver os conteúdos
  await admin.goto(`${BASE}/admin/permissoes`, {
    waitUntil: "domcontentloaded",
  });
  await hidratado(admin, "aside, nav");
  await admin.screenshot({
    path: `${TIROS}/conteudos-permissoes.png`,
    fullPage: true,
  });
  await admin
    .locator('select[name="perm:FUNCIONARIO:conteudos"]')
    .selectOption("ver");
  await admin.getByRole("button", { name: "Guardar permissões" }).click();
  await admin
    .getByText("permissão(ões) actualizada(s)")
    .waitFor({ timeout: 45000 });
  passo("Administrador altera permissões", true);

  const { contexto: ctxFunc, pagina: func } = await entrar(
    navegador,
    "domingos@ddress.ao",
    "funcionario123",
  );
  ...
  const botoes = await func
    .getByRole("button", { name: "Guardar texto" })
    .count();
  const bloqueado = await func.locator('input[name="title"]').isDisabled();
  passo(
    "Funcionário só vê os conteúdos",
    botoes === 0 && bloqueado,
    `botões ${botoes}, campo bloqueado ${bloqueado}`,
  );
  const equipa = await func.request.get(`${BASE}/admin/permissoes`, {
    maxRedirects: 0,
  });
  passo(
    "Funcionário não abre as permissões",
    equipa.status() !== 200,
    `estado ${equipa.status()}`,
  );
  await ctxFunc.close();
  ...
  await hidratado(admin, "aside, nav");
  passo(
    "Permissões repostas no padrão",
    (await admin
      .locator('select[name="perm:FUNCIONARIO:conteudos"]')
      .inputValue()) === "editar",
  );

  await admin.goto(`${BASE}/admin/auditoria?tipo=PAINEL`, {
    waitUntil: "domcontentloaded",
  });
  await hidratado(admin, "aside, nav");
  passo(
    "Auditoria regista as alterações do painel",
    await admin.getByText("Permissões repostas no padrão").first().isVisible(),
  );

  for (const rota of ["conteudos", "parceiros", "colecoes"]) {
    await admin.goto(`${BASE}/admin/${rota}`, {
      waitUntil: "domcontentloaded",
    });
    await hidratado(admin, "aside, nav");
    await admin.screenshot({
      path: `${TIROS}/conteudos-admin-${rota}.png`,
      fullPage: true,
    });
  }
