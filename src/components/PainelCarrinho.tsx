  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[1fr_20rem]">
      <ul className="space-y-6">
        {itens.map((i) => (
          <li key={i.chave} className="flex gap-4 border-b border-marfim-200 pb-6">
            <Link href={`/produto/${i.productSlug}`} className="shrink-0">
              {i.imagem ? (
                ...
                <img
                  src={i.imagem}
                  alt={i.nome}
                  className="h-28 w-21 bg-marfim-100 object-cover"
                  style={{ width: "5.25rem" }}
                />
              ) : (
                <div className="h-28 w-[5.25rem] bg-marfim-200" />
              )}
            </Link>
            ...
            <div className="flex-1">
              ...
              {i.exigeProva && !i.prova && (
                <p className="mt-1 text-sm text-ouro-escuro">
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  {i.tipo === "VENDA" ? (
                    <div className="flex items-center border border-marfim-300">
                      <button
                        type="button"
                        className="px-3 py-1 text-tinta-70 hover:bg-marfim-100"
                  ...
                  <button
                    type="button"
                    className="text-sm text-tinta-50 underline underline-offset-4 hover:text-ouro-escuro"
