...
export default function EditorVariantes({
  ...
  return (
    <section className="cartao p-5">
      ...
      {aAdicionar && (
        <form
          className="mt-4 grid gap-3 border border-marfim-300 bg-marfim-50 p-4 sm:grid-cols-5"
        ...
        <table className="tabela">
          ...
          <tbody>
            {variantes.map((v) =>
              aEditar === v.id ? (
                <tr key={v.id}>
                  <td colSpan={6} className="bg-marfim-50">
                    <form
                ...
                <tr key={v.id} className={v.active ? "" : "opacity-50"}>
                  ...
                  <td>
                    {!v.active ? (
                      <span className="selo bg-marfim-200 text-tinta-70">Desativado</span>
                    ) : v.rentalStock === 0 ? (
                  ...
                  <td>
                    <div className="flex gap-2">
                      <button
                        type="button"
                        className="text-xs text-ouro-escuro hover:underline"
                        onClick={() => {
                          setAEditar(v.id);
                          setAAdicionar(false);
                        }}
                      ...
                      {v.active && (
                        <button
                          type="button"
                          className="text-xs text-tinta-50 hover:text-ouro-escuro"
                          disabled={aProcessar}
