...
export default function SelectorProduto({ produto, imagem, variantes }: Props) {
  ...
  const [foraDeLuanda, setForaDeLuanda] = useState(false);
  ...
  function adicionarAoCarrinho(irParaCarrinho: boolean) {
    ...
    adicionar({
      ...
      dispensaProva: provaPedidaPelaPeca && foraDeLuanda,
  ...
  return (
    <div>
      ...
      {podeVender && podeAlugar && (
        <div className="mb-6 inline-flex border border-marfim-300">
            ...
            {disponiveis.map((v) => {
              ...
              return (
                ...
                >
                  {v.size}
                  {modo === "ALUGUER" && !livre && (
                    <span className="absolute -top-1.5 -right-1.5 h-2 w-2 rounded-full bg-ouro" />
                  )}
                </button>
              );
            })}
      ...
      {modo === "ALUGUER" && variante && (
        <div className="mb-6">
          ...
          {erroDatas && (
            <p className="mt-3 border-l-2 border-ouro bg-marfim-100 px-3 py-2 text-sm text-tinta">
              {erroDatas}
            </p>
          )}
          {orcamento && (
            <div className="mt-3 cartao p-4 text-sm">
              ...
              <div className="mt-2.5 flex justify-between border-t border-marfim-200 pt-2.5">
      ...
      <div className="mb-6">
            ...
            <div className="mb-4 space-y-2">
              <label className="flex cursor-pointer items-start gap-2.5 border border-marfim-300 bg-white p-3 text-sm">
                <input
                  ...
                  className="mt-0.5"
                  checked={!foraDeLuanda}
                  onChange={() => setForaDeLuanda(false)}
                />
                <span>
              ...
              <label className="flex cursor-pointer items-start gap-2.5 border border-marfim-300 bg-white p-3 text-sm">
                <input
                  ...
                  checked={foraDeLuanda}
                  onChange={() => {
                    setForaDeLuanda(true);
                    setProva(null);
                  }}
                />
                <span>
                  <span className="block font-medium">Resido fora de Luanda — dispensar a prova</span>
                  <span className="block text-tinta-70">
                    Confirma a morada e assina a declaração de responsabilidade no passo
                    seguinte.
                  </span>
                </span>
              </label>
        ...
        {provaPedidaPelaPeca && foraDeLuanda && (
          <p className="border-l-2 border-ouro bg-marfim-100 px-3 py-2 text-sm text-tinta-70">
            A prova fica dispensada. Confirme as medidas com atenção — a peça segue
            directamente para si, sem prova no ateliê.
          </p>
        )}
      </div>
      ...
      {/* ------------------------------------------------------ acções */}
      {mensagem && (
        <p className="mb-3 border-l-2 border-ouro bg-marfim-100 px-3 py-2 text-sm">{mensagem}</p>
      )}
