      ...
      <Link href={`/produto/${produto.slug}`} className="block">
        <div className="relative overflow-hidden bg-marfim-100">
          ...
          <div className="absolute top-3 left-3 flex flex-col items-start gap-1.5">
            {soAluguer && (
              <span className="selo bg-white/90 text-ouro-escuro">Só aluguer</span>
            )}
            ...
            {produto.compareAtPrice && produto.salePrice && produto.compareAtPrice > produto.salePrice && (
              <span className="selo bg-ouro text-preto">Promoção</span>
            )}
          </div>

          {indisponivel && (
            <div className="absolute inset-x-0 bottom-0 bg-tinta/85 px-3 py-2 text-center text-xs text-marfim-100">
